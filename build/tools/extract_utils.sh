...
function setup_vendor() {
    ...
    export AOST_ROOT="$3"
    if [ ! -d "$AOST_ROOT" ]; then
        echo "\$AOST_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$AOST_ROOT/$OUTDIR" ]; then
        mkdir -p "$AOST_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$AOST_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$AOST_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$AOST_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The UltraAOST Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The UltraAOST Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The UltraAOST Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The UltraAOST Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The UltraAOST Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$AOST_ROOT"/vendor/aost/build/tools/smali/baksmali.jar
        export SMALIJAR="$AOST_ROOT"/vendor/aost/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$AOST_ROOT"/vendor/aost/build/tools/"$HOST"/vdexExtractor
    fi
    ...
    if [ ! -f "$AOST_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$AOST_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$AOST_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$AOST_ROOT"/vendor/aost/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$AOST_ROOT"/"$OUTDIR"/radio
